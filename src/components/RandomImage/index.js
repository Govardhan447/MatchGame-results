...
const matchingListDetails = []

class RandomImage extends Component {
  state = {
    randomImageItem: updateList[0],
    notMatchingImage: true,
    imageUpdatedList: updateList,
    seconds: 60,
    matchingList: matchingListDetails,
  }
  ...
  startSeconds = () => {
    ...
    console.log(seconds)
    if (seconds === 0) {
      this.setState({notMatchingImage: false})
    } else {
  ...
  getMatchingList = isMatching => {
    ...
    if (isMatching === 'notMatching') {
      ...
      this.setState({notMatchingImage: false, seconds: 0})
    } else {
      const randamImageId = Math.round(Math.random() * 30)
      const randomImage = updateList.filter(item => randamImageId === item.id)

      this.setState(prevState => ({
        randomImageItem: randomImage[0],
        matchingList: [...prevState.matchingList, isMatching],
      }))
    }
  }

  getPlaymode = () => {
    const {randomImageItem, imageUpdatedList} = this.state
    ...
    return (
      <>
        ...
        <MatchGame
          imageListItems={imageUpdatedList}
          getMatchingList={this.getMatchingList}
          randomImageDetails={randomImageItem}
  ...
  getYourScore = () => {
    const {matchingList} = this.state
    const score = matchingList.length
    return (
      <div className="scoreboard-container">
        ...
        <button
          className="btn-playagain score-heading"
          type="button"
  ...
  render() {
    const {matchingList, notMatchingImage, seconds} = this.state
    const score = matchingList.length
      ...
      <div className="bg-container">
        <nav className="navbar-container">
          ...
          <div className="score-container">
            <p className="score">
              Score: <span className="span-num">{score}</span>
            </p>
            <div className="timer-logo-container">
              <img
                ...
                alt="timer"
              />
              <p className="span-num">{seconds} Sec</p>
            </div>
          </div>
        </nav>
        {notMatchingImage ? this.getPlaymode() : this.getYourScore()}
      </div>
    )
  }
}
